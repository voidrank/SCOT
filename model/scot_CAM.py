...
from . import resnet
from .resnet_discobox import ResNet
from .fpn_discobox import FPN
...
class SCOT_CAM(nn.Module):
    ...
    def __init__(self, backbone, hyperpixel_ids, benchmark, device, cam):
        ...
        if backbone == 'resnet50':
            ...
            self.neck = FPN([256, 512, 1024, 2048], 256, 5)
            weights = torch.load("discobox_res101.pth")
    ...
    def extract_hyperpixel(self, img, maptype, bbox, mask, backbone="resnet101"):
        ...
        hyperfeats, rfsz, jsz, feat_map, fc = self.extract_intermediate_feat(img.unsqueeze(0), return_hp=True,
                                                                             backbone=backbone)
    ...
    def get_FCN_map(self, img, feat_map, fc, sz):
        ...
        for scale in scales:
            if scale * scale * sz[0] * sz[1] > 1200 * 800:
                scale = 1.5
            img = F.interpolate(img, (int(scale * sz[0]), int(scale * sz[1])), None, 'bilinear', True)  # 1x3xHxW
            _, _, _, feat_map, fc = self.extract_intermediate_feat(img,return_hp=False,backbone='fcn101')
            ...
            mask_map = F.interpolate(mask_map.unsqueeze(0).double(), (sz[0], sz[1]), None, 'bilinear', True)[
                0, 0]  # HxW
